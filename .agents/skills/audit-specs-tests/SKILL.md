...
AltTab co-locates tests and a living spec next to the source they cover. The convention (see the plan in `.Codex/plans/` and `src/pro/license/` as a worked example) is a **triad** per concept:
